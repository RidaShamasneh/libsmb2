                     ...
                     void *command_data, void *private_data)
{
        ...
        if (status != SMB2_STATUS_SUCCESS) {
                smb2_set_nterror(smb2, status, "%s", nterror_to_str(status));
        } else if (command_data != NULL) {
                output = ((struct smb2_ioctl_reply *)command_data)->output;
        }
...
smb2_copychunk_async(struct smb2_context *smb2,
                     ...
                     smb2_command_cb cb, void *cb_data)
{
        return smb2_copychunk_submit_async(smb2, resume_key, dstfh, chunks,
                                           chunk_count, cb, cb_data);
}
                                    ...
                                    void *command_data, void *private_data)
{
        ...
        ret = smb2_copychunk_submit_async(smb2, command_data, copy_data->dstfh,
                                          copy_data->chunks,
